...
const Customer_registration = () => {
        ...
        .then((json) => {
          ...
          if (json.data.token) {
            ...
            Swal.fire({
              ...
              text: 'Shop have successfully logged in.',
            });
          } else {
  ...
  const Signup = async (e) => {
    ...
    try {
      ...
      setError('An error occurred during shop registration. Please try again.');
    }
  };
  //console.log('json.data is =>', json.data)
  
  /*console.log('After shop login we get on registration => ', Cookies.get('token'))
  console.log('After shop login we get on registration shop data => ', Cookies.get('shop'))
  const shop =  JSON.parse(Cookies.get('shop'));
...
// Check if the shop object exists and log specific properties
if (shop) {
  console.log('Username:', shop.username);
  // Add more lines to log other properties as needed
} else {
  console.log('Shop object not found or undefined.');
}
  console.log('Decoded token of shop login is =>', JWT.decode(token))
